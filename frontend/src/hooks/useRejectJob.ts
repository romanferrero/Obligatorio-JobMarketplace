...
export function useRejectJob(jobId: bigint) {
  ...
  const reject = (reason: string) => write([jobId, keccak256(toBytes(reason || "Rechazado"))]);

  return { reject, status, errorMessage, reset };
}
